...
int main(int argc, char **argv) {
  // 0. Load .env file if available
  load_env_file(".env");

  // 1. Load API Key securely
  const char *env_api_key = std::getenv("GROQ_API_KEY");
  if (!env_api_key) {
    std::cerr << "[CRITICAL] GROQ_API_KEY not found in environment variables."
              << std::endl;
    std::cerr
        << "Run: export GROQ_API_KEY='your_key_here' OR create a .env file"
        << std::endl;
    return 1;
  }
  ...
  std::string clean_analysis = parse_groq_response(ai_response);

  // 5. Output (The "Portfolio" Look)
  std::cout << "\n=============================================" << std::endl;
  std::cout << "           SENTRY-AI ANALYSIS REPORT         " << std::endl;
  std::cout << "=============================================" << std::endl;

  std::cout << clean_analysis << std::endl;
